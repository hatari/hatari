/*
  ...
  even through-out.
  NOTE: If the emulator runs slower than 50fps it cannot update the buffers, but the DirectSound thread still
...
unsigned long ChannelFreq[3], EnvelopeFreq, NoiseFreq;          /* Current frequency of each channel A,B,C,Envelope and Noise */
int ChannelAmpDecayTime[3];                                     /* Store counter to show if amplitude is changed to generate 'samples' */
int Envelope[SAMPLES_BUFFER_SIZE],Noise[SAMPLES_BUFFER_SIZE];   /* Current sample for this time period */
/* Output channel data */
int Channel_A_Buffer[SAMPLES_BUFFER_SIZE],Channel_B_Buffer[SAMPLES_BUFFER_SIZE],Channel_C_Buffer[SAMPLES_BUFFER_SIZE];
/* Use table to convert from (A+B+C) to clipped 'unsigned char' for DirectSound buffer */
...
int OddEvenSoundFrame = 0;                                      /* 11Khz does no divide nicely(remainder 0.5), so use this */
...
int SquareWave[16] = { 127,127,127,127,127,127,127,127, -128,-128,-128,-128,-128,-128,-128,-128 };
/* LogTable */
int LogTable[256];
int LogTable16[16];
int *pEnvelopeLogTable = &LogTable[128];
...
void Sound_Reset(void)
{
  ...
  // Clear buffer, passed to DirectSound
...
{
  // Clear buffer, passed to DirectSound
  Memory_Clear(MixBuffer,MIXBUFFER_SIZE);
}
...
/*
  Create Log tables
*/
void Sound_CreateLogTables(void)
{
  float a;
  int i;
...
*/
void Sound_CreateEnvelopeShape(ENVSHAPE *pEnvShape,int *pEnvelopeValues)
{
  int i,j,Value;
...
*/
void Sound_CreateEnvelopeShapes(void)
{
  int i;
...
//-----------------------------------------------------------------------
/*
  Copy emulation sound buffer to DirectSound
  This routine is called from Windows thread every 1/50th of a second
*/
void Sound_PassYMSamplesToDirectSound(void)
{
  ...
  if (bDirectSoundWorking) {
    ...
    if (bEmulationActive!=EMULATION_ACTIVE) {
      ...
      DAudio_WriteSamplesIntoBuffer(MixBuffer,CompleteSoundBuffer,SoundPlayBackFreqFrameLengths[OutputAudioFreqIndex][OddEvenSoundFrame],RAMP_DOWN);
      OddEvenSoundFrame ^= TRUE;
    }
    else {
      // Now pass completed buffer to DirectSound, toggle odd/even frame(for 11Khz timing)
      DAudio_WriteSamplesIntoBuffer(MixBuffer,CompleteSoundBuffer,SoundPlayBackFreqFrameLengths[OutputAudioFreqIndex][OddEvenSoundFrame],RAMP_UP);
      OddEvenSoundFrame ^= TRUE;
    }
  }
}
...
*/
void Sound_GenerateEnvelope(unsigned char EnvShape, unsigned char Fine, unsigned char Coarse)
{
  int *pEnvelopeValues;
...
*/
void Sound_GenerateNoise(unsigned char MixerControl, unsigned char NoiseGen)
{
  int NoiseValue;
...
*/
void Sound_GenerateChannel(int *pBuffer, unsigned char ToneFine, unsigned char ToneCoarse,unsigned char Amplitude,unsigned char MixerControl,unsigned long *pChannelFreq,int MixMask)
{   
...
void Sound_GenerateSamples(void)
{
  ...
  if (nSamplesToGenerate>0) {
    ...
    for(i=0; i<nSamplesToGenerate; i++)
      MixBuffer[(i+ActiveSoundBuffer)&MIXBUFFER_LENGTH] = pMixTable[(*pChannelA++) + (*pChannelB++) + (*pChannelC++)];
    ActiveSoundBuffer = (ActiveSoundBuffer+nSamplesToGenerate)&MIXBUFFER_LENGTH;
...
void Sound_Update_VBL(void)
{
  ...
  CompleteSoundBuffer = (CompleteSoundBuffer+SAMPLES_PER_FRAME)&MIXBUFFER_LENGTH;
